"""
Cleans Timely_and_Effective_Care_-_Hospital.csv

Execution: 
    python timely_care_cleaning.py
Output: 
    1) timely_care_cleaned.csv - ready to be imported into MySQL
"""
...
timely_filepath = '../../../datasets/hospital/Timely_and_Effective_Care_-_Hospital.csv'
...
df = spark.read.options(header='True', inferSchema='True')\
    .csv(timely_filepath).alias('df')
ids = spark.read.options(header='True', inferSchema='True')\
    .csv(ids_filepath).alias('ids')
...
df2.coalesce(1).write.option("header","true")\
    .csv("../../../datasets/hospital/timely_care_cleaned.csv")